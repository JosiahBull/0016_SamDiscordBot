...
const help = [
    ...
    {
        command: 'bfcount',
        title: '***REMOVED*** Boyfriends',
        description: `number of ***REMOVED***'s boyfriends.`
    },
    {
        command: 'hide',
        title: 'Hide Messages',
...
const TodoDataStore = function (initPath) {
    ...
    this.saveFile = async () => await fs.writeFileSync(path.***REMOVED***in(__dirname, savePath), JSON.stringify(this.data), {encoding: 'utf-8'});
    this.loadFile = async () => this.data = await JSON.parse(fs.readFileSync(path.***REMOVED***in(__dirname, savePath)));
...
const ImageLoader = function(command, url, author, dateAdded = Date.now()) {
    ...
    //         let outpath = path.***REMOVED***in(__dirname, '/images/downloading', `${this.name}.jpg`);
    //         this.ext = '.jpg';
    ...
    //     const writer = fs.createWriteStream(path.***REMOVED***in(__dirname, '/images/downloading', this.name + this.ext));
    //     this.offlineUrl = path.***REMOVED***in(__dirname, '/images/downloading', this.name + this.ext);
    ...
    //     this.offlineUrl = path.***REMOVED***in(__dirname, 'images/', this.name + '.jpg'); //Set url to new path.
...
const ImageReplyDataStore = function(initPath) {
    ...
    async function checkType(image) {
        if (image.ext === '.png') {
            console.log('Image was png, converting to jpg.');
            let outpath = path.***REMOVED***in(__dirname, '/images/downloading', `${image.name}.jpg`);
            image.ext = '.jpg';
    ...
    async function compress(image) {
        ...
        image.offlineUrl = path.***REMOVED***in(__dirname, 'images/', image.name + image.ext); //Set url to new path.
        return image;
    }

    async function downloadImage(image) {
        console.log(`Downloading image from link: ${image.onlineUrl}, which has extension ${image.ext}\n`);
        const writer = fs.createWriteStream(path.***REMOVED***in(__dirname, '/images/downloading', image.name + image.ext));
        image.offlineUrl = path.***REMOVED***in(__dirname, '/images/downloading', image.name + image.ext);
        let response = await axios({
            url: image.onlineUrl,
            method: 'GET',
            responseType: 'stream'
    ...
    if (content.substring(0, triggerChar.length) === triggerChar) {
        ...
        switch (args[0]) {
            ...
            case 'bfcount': {
                ...
                message.channel.send(`***REMOVED*** currently has ${configStore.data.bfcount} boyfriends, that's crazy!`);
                break;
            }
            case 'hide': {
