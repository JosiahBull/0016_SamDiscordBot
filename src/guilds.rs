...
use crate::{
    commands::{application_command, autocomplete, command, interaction as handle_interaction}, messages::non_command_message, state::AppState
};
    ...
    match interaction {
            ...
            match res {
                Ok(response) => {
                    ...
                    if let Some(resp) = response.generate_response() {
                        if let Err(e) = raw_command.create_response(&context.http, resp).await {
        ...
        Interaction::Autocomplete(interaction) => {
            ...
            if let Err(e) = interaction
                .create_response(&context.http, CreateInteractionResponse::Autocomplete(resp))
                .await
            {
...
/// Manages a guild, interacting with and responding to events.
pub struct GuildHandler {
    ...
    context: Context,
    /// Shared State of the Bot
    app_state: AppState,
    /// user_id of the bot
    bot_user_id: u64,
    /// Internal task managing the guild once started
    handle: Option<JoinHandle<()>>,
    /// Receiving end of the internal communication channel
    internal_rx: Arc<RwLock<UnboundedReceiver<DiscordEvent>>>,
    /// Sending end of the internal communication channel
    ...
    pub fn start(&mut self) {
        if self.handle.is_none() {
            ...
            self.handle = Some(tokio::task::spawn(async move {
                ...
                while let Err(e) = guild.set_commands(&context.http, &application_command()).await {
